...
<head>
    ...
    if (isset($_POST["btn_add"])) {
        ...
        if (trim($brandid) == "") {
            ...
            $sq = "select * from public.store where storename='$brandname' or storeid='$brandid'";
            $res = pg_query($conn, $sq);
            if (pg_num_rows($res) == 0) {
                pg_query($conn, "INSERT INTO public.store(storeid, storename, address) VALUES ('$brandid','$brandname','$address')")
                    or die(pg_result_error($conn));
                echo "<script type='text/javascript'>alert('Add Store Successful');</script>";
                echo "<script> location.href='admin.php?page=store'; </script>";
                exit;
            } else {
                echo "<script type='text/javascript'>alert('Add Brand Failing');</script>";
                echo "<script> location.href='admin.php?page=add_store'; </script>";
                exit;
            }
        }
    }
...
<body>
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        ...
        <div class="btn-cancel">
            <a class="btn btn-danger" href="admin.php?page=store" role="button"><i class="fas fa-times"></i><Span> Cancel</Span></a>
        </div>
